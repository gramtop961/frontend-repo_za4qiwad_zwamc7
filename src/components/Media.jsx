export default function Media() {
  ...
  return (
    <section id="media" className="relative bg-black">
      <div className="absolute inset-0 bg-[radial-gradient(800px_400px_at_50%_-10%,rgba(239,68,68,0.12),transparent)]" />
      <div className="relative mx-auto max-w-7xl px-6 py-24">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2">
          {videos.map((v) => (
            <figure key={v.id} className="group overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03]">
              <video
        ...
        <div className="mt-10 grid grid-cols-2 md:grid-cols-4 gap-4">
          {images.map((src, idx) => (
            <div key={idx} className="group relative aspect-[4/3] overflow-hidden rounded-xl border border-white/10 bg-white/[0.03]">
              <img
